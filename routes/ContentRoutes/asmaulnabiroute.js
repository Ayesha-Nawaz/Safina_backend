...
const AsmaulNabi = require("../../models/contentModels/asmaulnabimodel");

// ➤ GET all names
router.get("/names", async (req, res) => {
  try {
    const names = await AsmaulNabi.find();
...
router.get("/:id", async (req, res) => {
  try {
    const name = await AsmaulNabi.findById(req.params.id );
...
router.post("/name", async (req, res) => {
    ...
    try {
      const newName = new AsmaulNabi(req.body);
...
//------------------------------
    ...
    router.post("/names", async (req, res) => {
      ...
      try {
          ...
          const savedNames = await AsmaulNabi.insertMany(names);
...
router.put("/:number", async (req, res) => {
  try {
    const updatedName = await AsmaulNabi.findOneAndUpdate(
...
router.delete("/:number", async (req, res) => {
  try {
    const deletedName = await AsmaulNabi.findOneAndDelete({ number: req.params.number });
